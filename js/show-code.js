// On Click Page Opening 
function imclk(sc){
    window.open(document.getElementById(sc).src, '_blank');
}
// Link Format  
function getquote(quote){
    window.open(`get-quote.html?name=${document.getElementById(quote).textContent.split(" ").join("+")}`, '_self');
}
...
window.addEventListener('load' , () => {
    ...
    if (topic == 'living-room'){
        ...
        // Category wise price listings        
        const prilist = ["", "243,965", "86,039", "154,371", "132,935", "114,997", "129,919", "101,274", "141,298", "148,505", "56,137"];
        // loop is used to get the images for the titles and pricings we have added above
        for (let i = 1; i < 11; i++) {
